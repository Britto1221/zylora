...
def create_public_lead(
    ...
    set_public_tenant_context(db, site.tenant_id)
    ...
    lead = Lead(
        ...
        if payload.whatsapp_consent or payload.marketing_consent
        else None,
...
def list_leads(
    ...
    return {
        "items": [model_dict(item) for item in leads],
        "total": total,
        "page": page,
        "pageSize": page_size,
    }
...
def add_note(
    ...
    if not lead:
        raise HTTPException(status_code=404, detail="Lead not found.")
    note = LeadNote(tenant_id=tenant_id, lead_id=lead.id, author_user_id=user.id, body=payload.body)
    db.add(note)
    record_audit(
        db,
        actor_user_id=user.id,
