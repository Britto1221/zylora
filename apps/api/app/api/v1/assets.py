...
def public_asset(asset_id: UUID, db: Session = Depends(get_db)):
    ...
    from app.modules.storage.service import local_path
...
def create_presigned_upload(
    ...
    db.add(asset)
    db.flush()
    upload = presigned_upload(key=key, mime_type=payload.mime_type)
    record_audit(
        db,
        ...
        payload={"filename": name, "mime_type": payload.mime_type},
    )
    db.commit()
    return {
...
async def local_upload(
    ...
    if len(data) != asset.size_bytes or len(data) > settings.max_upload_bytes:
    ...
    asset.status = "READY"
    asset.public_url = f"/api/v1/assets/public/{asset.id}"
    record_audit(
        db,
        actor_user_id=user.id,
        tenant_id=tenant_id,
